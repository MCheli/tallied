...
def _reset(db):
    db.execute(text("DELETE FROM sync_jobs"))
    db.execute(text("DELETE FROM monarch_account_configs"))
    db.execute(text("DELETE FROM monarch_links"))
    # Wipe SimpleFIN too so it doesn't bleed into scheduled-path which
    # iterates ALL providers.
    db.execute(text("DELETE FROM simplefin_account_configs"))
    db.execute(text("DELETE FROM simplefin_links"))
    db.execute(text("DELETE FROM transactions WHERE id LIKE 'monarch-%' OR id LIKE 'simplefin-%'"))
    db.execute(text("DELETE FROM balance_snapshots WHERE source IN ('monarch', 'simplefin')"))
    db.execute(text("DELETE FROM accounts WHERE id LIKE 'monarch-%' OR id LIKE 'simplefin-%' OR id = 'ghost-account'"))
    db.commit()
...
async def scenario_happy_path():
    ...
    with patch("app.services.sync_scheduler._get_monarch_client", return_value=mm):
        job_id, _created = create_job_row(SCHEMA, provider="monarch", trigger="manual")
        await run_sync_with_job(SCHEMA, job_id, "monarch")
...
def scenario_watchdog_reaps_stuck():
    ...
    try:
        ...
        stuck = MonarchSyncJob(
            provider="monarch", status="running", trigger="manual",
...
def scenario_unique_running_index():
    ...
    a, a_created = create_job_row(SCHEMA, provider="monarch", trigger="manual")
    b, b_created = create_job_row(SCHEMA, provider="monarch", trigger="manual")
...
async def scenario_notify_listener_e2e():
    ...
    try:
        ...
        with patch("app.services.sync_scheduler._get_monarch_client", return_value=mm):
            job_id, created = create_job_row(SCHEMA, provider="monarch", trigger="manual")
            assert created, "expected new job to be created"
            notify_sync_request(SCHEMA, "monarch")
...
async def scenario_bulk_existence_check():
    ...
    with patch("app.services.sync_scheduler._get_monarch_client", return_value=mm):
        j1, _ = create_job_row(SCHEMA, provider="monarch", trigger="manual")
        await run_sync_with_job(SCHEMA, j1, "monarch")
    ...
    with patch("app.services.sync_scheduler._get_monarch_client", return_value=mm):
        j2, _ = create_job_row(SCHEMA, provider="monarch", trigger="manual")
        await run_sync_with_job(SCHEMA, j2, "monarch")
